...
<div class="box box-danger">
    ...
    <div class="box-header with-border">
        ...
        <h3 class="box-title">
            ...
            Period : {{$shift->duty_finish_time->diffinHours($shift->duty_start_time)}}
            :{{sprintf('%02d',fmod($shift->duty_finish_time->diffinMinutes($shift->duty_start_time),60))}}
        ...
        <div class="box-tools pull-right">
            @if ($shift->locked_flag == true)
                <button type="button" class="btn btn-default btn-xs" data-toggle="tooltip"
                        title="Shift can be unlocked via your User page <Link here>" data-placement="left">Locked
                </button>
            @else
                <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#myModal2">Edit
                </button>
            @endif
        </div><!-- /.box-tools -->
